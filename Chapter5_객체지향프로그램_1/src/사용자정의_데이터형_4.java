...
public class 사용자정의_데이터형_4 {
	...
	public static void main(String[] args) {
		...
		// 클래스 데이터형 => 일반 데이터형과 동일 (단지 한메모리에 여러개의 다른 데이터들을 모아둔거임!)
		...
		for(Music m:music)
		{
			System.out.println(i+"."+m.title+" - "+m.singer);
			i++;
		}
		...
		if(s.equals("유지"))
			...
			s = "▲"+music[no-1].modify;
		else if(s.endsWith("하강"))
			s = "▼"+music[no-1].modify;
		System.out.println("순위 : "+no + "위("+s+")");
